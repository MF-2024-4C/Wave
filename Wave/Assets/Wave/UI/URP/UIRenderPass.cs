...
namespace Wave.UI
{
    public class UIRenderPass : ScriptableRenderPass
    {
        ...
        private readonly ProfilingSampler _uiDefaultSampler = new("UIDefault");
        ...
        public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            RenderingUtils.ReAllocateIfNeeded(ref _blurCaptureRT, rtDescriptor, FilterMode.Bilinear,
                TextureWrapMode.Clamp, name: ShaderID.BlurCaptureRTName);
        }
            ...
            */
            ...
            using (new ProfilingScope(cmd, _uiBlurBelowSampler))
            {
                ...
                var rendererListParams =
                    new RendererListParams(renderingData.cullResults, drawingSettings, _belowFilteringSettings);
            ...
            using (new ProfilingScope(cmd, _uiDefaultSampler))
            {
                ...
                var rendererListParams = new RendererListParams(renderingData.cullResults, drawingSettings,
                    _defaultFilteringSettings);
                var list = context.CreateRendererList(ref rendererListParams);
                cmd.DrawRendererList(list);
            }
            ...
            using (new ProfilingScope(cmd, _uiBlurAboveSampler))
            {
                unsafe
                {
                    ...
                    var rendererListParams = new RendererListParams(renderingData.cullResults, drawingSettings,
                        _aboveFilteringSettings);
                    fixed (RenderStateBlock* stateBlocks = &_stateBlock)
                    {
                        var array = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<RenderStateBlock>(
                            stateBlocks,
                            1,
                            Allocator.None
                        );
...
// 安全性チェック用のAtomicSafetyHandleを設定する
                        ...
                        rendererListParams.stateBlocks = array;

                        var shaderTag = ShaderTagId.none;
                        var tagValues =
                            NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<ShaderTagId>(&shaderTag, 1,
                                Allocator.None);
#if ENABLE_UNITY_COLLECTIONS_CHECKS
// 安全性チェック用のAtomicSafetyHandleを設定する
                        NativeArrayUnsafeUtility.SetAtomicSafetyHandle(ref tagValues,
                            AtomicSafetyHandle.GetTempMemoryHandle());
#endif
