...
public unsafe partial class ZombieSpawnAreaConfig : AssetObject
{
    ...
    public virtual void Spawn(Frame f, FPVector3 zombieSpawnAreaPosition)
    {
        var zombie = f.Create(Prototype);
        var zombieTransform = f.Unsafe.GetPointer<Transform3D>(zombie);
        zombieTransform->Position = GetRandomRadiusPosition(f.RNG, zombieSpawnAreaPosition);
    }
    
    public FPVector3 GetRandomRadiusPosition(RNGSession* RNG,FPVector3 center)
    {
